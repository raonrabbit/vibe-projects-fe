import { PALETTE, SEMANTIC_LIGHT, SEMANTIC_DARK, SPACING, RADIUS, TYPE_SCALE, resolveColor } from './tokens-data'
...
function semanticRow(tokenName: string): FrameNode {
  ...
  lightRect.fills = [{ type: 'SOLID', color: { r: lc.r, g: lc.g, b: lc.b }, opacity: lc.a }]
  ...
  darkRect.fills = [{ type: 'SOLID', color: { r: dc.r, g: dc.g, b: dc.b }, opacity: dc.a }]
...
const SAMPLE_TEXTS: Record<string, string> = {
  Display:  '모두가 나답게 일하고 즐겁게 성장할 수 있도록',
  Title:    'AI 기반 채용 플랫폼',
  Heading:  '채용 공고',
  Headline: '스킬 선택',
  Body:     '원티드랩은 차별화된 데이터를 기반으로 각자에게 가장 잘 맞는 커리어 경로를 설계합니다.',
  Label:    '이메일',
  Caption:  '합격보상금 100만원',
}
  ...
  for (const style of TYPE_SCALE) {
    ...
    if (style.category !== currentCategory) {
      ...
      if (currentCategory !== 'Display') {
        ...
        spacer.resize(700, 1)
        spacer.fills = [{ type: 'SOLID', color: { r: 0.929, g: 0.937, b: 0.949 } }] // #e5e7eb
        card.appendChild(spacer)
      }

      const catHeader = vframe(4)
      catHeader.counterAxisAlignItems = 'MIN'
      catHeader.appendChild(txt(currentCategory, 13, 'Semi Bold', '#111827'))
      catHeader.appendChild(txt(CATEGORY_DESCS[currentCategory] ?? '', 11, 'Regular', '#9ca3af'))
    ...
    meta.appendChild(txt(`${style.size}px · ${style.weight} · lh ${style.lineHeight}`, 10, 'Regular', '#9ca3af'))
...
function createRadiusSection(): FrameNode {
  ...
  for (const [key, value] of Object.entries(RADIUS)) {
    ...
    group.appendChild(txt(value === 9999 ? '∞' : `${value}px`, 10, 'Regular', '#9ca3af'))
