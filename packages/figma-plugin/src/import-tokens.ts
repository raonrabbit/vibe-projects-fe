import { PALETTE, SEMANTIC_LIGHT, SEMANTIC_DARK, SPACING, RADIUS, FONT_SIZES, TYPE_SCALE } from './tokens-data'
...
function getOrCreateVariable(name: string, collection: VariableCollection, type: VariableResolvedDataType): Variable {
...
export async function importAllTokens(onProgress: Progress) {
  ...
  for (const [colorName, shades] of Object.entries(PALETTE)) {
    for (const [shade, hexValue] of Object.entries(shades)) {
      const variable = getOrCreateVariable(`${colorName}/${shade}`, paletteColl, 'COLOR')
  ...
  const setValue = (variable: Variable, modeId: string, raw: string) => {
    ...
    if (ref) {
      ...
      if (paletteId) { variable.setValueForMode(modeId, { type: 'VARIABLE_ALIAS', id: paletteId }); return }
    }
    variable.setValueForMode(modeId, hexToRgba(raw))
  }

  const allKeys = new Set([...Object.keys(SEMANTIC_LIGHT), ...Object.keys(SEMANTIC_DARK)])
  for (const key of allKeys) {
    const lightVar = getOrCreateVariable(key, semanticColl, 'COLOR')
    if (SEMANTIC_LIGHT[key]) setValue(lightVar, lightModeId, SEMANTIC_LIGHT[key])
    const darkVar = sharedCollection ? lightVar : getOrCreateVariable(key, darkColl, 'COLOR')
    if (SEMANTIC_DARK[key]) setValue(darkVar, darkModeId, SEMANTIC_DARK[key])
  }
  ...
  for (const [key, value] of Object.entries(SPACING)) {
    getOrCreateVariable(key, spacingColl, 'FLOAT').setValueForMode(spacingModeId, value)
  }
  ...
  for (const [key, value] of Object.entries(RADIUS)) {
    getOrCreateVariable(key, radiusColl, 'FLOAT').setValueForMode(radiusModeId, value)
  }
  ...
  for (const [key, value] of Object.entries(FONT_SIZES)) {
    getOrCreateVariable(key, typoColl, 'FLOAT').setValueForMode(typoModeId, value)
  }
  ...
  for (const style of TYPE_SCALE) {
    ...
    const existing = figma.getLocalTextStyles().find(s => s.name === styleName)
    ...
    ts.letterSpacing = { value: style.letterSpacing * 100, unit: 'PERCENT' }
  }
}
