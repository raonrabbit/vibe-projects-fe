...
export function DepartureMap({
  ...
  const draw = useCallback(() => {
    ...
    const fromA = from ? (AIRPORTS.find((a) => a.iata === from) ?? null) : null;
    ...
    for (const c of candidates) {
      ...
      ctx.fill();
      ctx.strokeStyle = sel ? "rgba(251,191,36,0.3)" : "rgba(251,191,36,0.1)";
      ctx.lineWidth = 7 / t.k;
      ctx.stroke();
    }
    ...
    const drawDot = (a: Airport, color: string, glow: string) => {
      ...
      if (!p) return;
      const g = ctx.createRadialGradient(p[0], p[1], 0, p[0], p[1], 16 / t.k);
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full overflow-hidden rounded-2xl bg-fr-base select-none"
    ...
    >
      ...
      <div className="pointer-events-none absolute right-3 bottom-2.5">
        <span className="hidden text-[9px] tracking-widest text-white/18 uppercase sm:inline">
          스크롤 확대 · 드래그 이동
        </span>
        <span className="text-[9px] tracking-widest text-white/18 uppercase sm:hidden">
          드래그 이동 · 핀치 확대
        </span>
      </div>
    </div>
