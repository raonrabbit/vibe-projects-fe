...
function SceneContent({
  ...
  useFrame(() => {
    ...
    if (cameraSnapshotRef) {
      offsetTemp.current.subVectors(controls.object.position, controls.target);
  ...
  return (
    <>
      <FlightSky hour={hour} skyDistance={LIVE_SKY_DISTANCE} centerOnCamera />
      <SessionFlightArc
        fromLat={fromLat}
        fromLng={fromLng}
        toLat={toLat}
...
export default function LiveMapCanvas({
  ...
  const initPlanePos = useMemo(
    () =>
      computePlanePosition(fromLat, fromLng, toLat, toLng, progress, MAP_SCALE),
    [],
  );
  ...
  return (
    <div className="absolute inset-0 z-20 bg-[#070e1a]">
      ...
      <div
        className={`pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-[#070e1a] transition-opacity duration-700 ${
          mapReady ? "opacity-0" : "opacity-100"
        }`}
      >
        <div className="flex flex-col items-center gap-3">
          <div className="h-7 w-7 animate-spin rounded-full border-2 border-white/10 border-t-sky-400/60" />
      ...
      <Canvas
        camera={{
          ...
          fov: 48,
          near: 0.005,
          far: Math.max(LIVE_SKY_DISTANCE * 1.2, MAP_W * MAP_SCALE * 4),
        }}
        dpr={isMobile ? 1 : [1, 2]}
        gl={{
          antialias: !isMobile,
